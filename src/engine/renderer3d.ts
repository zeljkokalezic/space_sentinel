...
// Loose alias for per-frame mesh juggling — three.js objects are dynamically
// mutated (material/geometry/userData), so we keep them permissive internally.
/* eslint-disable @typescript-eslint/no-explicit-any */
type AnyMesh = any;

/** The Three.js scene bundle created by initThreeScene and passed to draw3DFrame. */
export interface ThreeScene {
  scene: AnyMesh;
  camera: AnyMesh;
  renderer: AnyMesh;
  g: Record<string, AnyMesh>;
  m: Record<string, AnyMesh>;
  meshes: Map<unknown, AnyMesh>;
}
...
const _activeKeys = new Set<unknown>();
const _sharedGeos = new Set<AnyMesh>();
const _sharedMats = new Set<AnyMesh>();
function initSharedSets(geoms: Record<string, AnyMesh>, mats: Record<string, AnyMesh>): void {
  _sharedGeos.clear();
  _sharedMats.clear();
 ...
 */
const getMesh = (obj: unknown, meshes: Map<unknown, AnyMesh>, scene: AnyMesh, createFn: () => AnyMesh): AnyMesh => {
  if (!meshes.has(obj)) { const m = createFn(); scene.add(m); meshes.set(obj, m); }
  _activeKeys.add(obj);
  return meshes.get(obj);
};

export const raycastToPlane = (clientX: number, clientY: number, camera: AnyMesh): { x: number; y: number } | null => {
...
export const projectToScreen = (camera: AnyMesh, wx: number, wy: number, wz = 0): { x: number; y: number; visible: boolean } => {
...
export const initThreeScene = (containerEl: HTMLElement | null): ThreeScene => {
...
export const draw3DFrame = (threeObj: ThreeScene, gState: GameState): void => {
  const g = gState as any;
  ...
  if (!pm.userData.shieldMesh) pm.userData.shieldMesh = pm.children.find((c: AnyMesh) => c.name === 'shield');
  if (!pm.userData.turretsGroup) pm.userData.turretsGroup = pm.children.find((c: AnyMesh) => c.name === 'turrets');
  ...
  if (pm.userData.skinIdx !== skinIdx) {
    ...
    const updateMaterials = (obj: AnyMesh, color: number): void => {
      if (obj.material) obj.material.color.setHex(color);
      if (obj.children) obj.children.forEach((c: AnyMesh) => updateMaterials(c, color));
    };
    pm.children.forEach((child: AnyMesh) => {
  ...
  if (g.playerIFrames && g.playerIFrames.active) {
    ...
    pm.userData.hullMeshes.forEach((m: AnyMesh) => { if (m) m.visible = blinkVisible; });
  } else {
    // Ensure hull meshes are visible when not in i-frames
    pm.userData.hullMeshes.forEach((m: AnyMesh) => { if (m && !m.visible) m.visible = true; });
  }
  ...
  if (turretsGroup) {
    ...
    if (pm.userData.levelsHash !== hash) {
      ...
      turretsGroup.children.forEach((child: AnyMesh) => {
        child.traverse((obj: AnyMesh) => {
          if (obj.geometry) obj.geometry.dispose();
          if (obj.material) {
            if (Array.isArray(obj.material)) obj.material.forEach((m: AnyMesh) => m.dispose());
      ...
      const addTurret = (x: number, y: number, isDouble: boolean): void => {
      ...
      const addMissilePod = (x: number, y: number): void => {
    ...
    turretsGroup.children.forEach((t: AnyMesh) => { if (t.userData.isAiming) t.rotation.z = (worldAim - Math.PI / 2) - pm.rotation.z; });
  }
  ...
  if (g.boss && g.boss.active && g.boss.hp > 0) {
    ...
    if (bdx * bdx + bdy * bdy <= renderDistSq) {
      const bm = getMesh('boss', meshes, scene, () => {
        ...
        const geoMap: Record<string, () => AnyMesh> = {
          box: () => new THREE.BoxGeometry(1, 1, 1),
  ...
  if (g.boss && g.boss.decoy && g.boss.decoy.active) {
    ...
    if (ddx * ddx + ddy * ddy <= renderDistSq) {
      ...
      dm.children.forEach((child: AnyMesh) => {
  ...
  if (g.escort.active && g.escort.hp > 0) {
    ...
    destMesh.children.forEach((c: AnyMesh) => c.rotation.z += 0.01);
  }
  ...
  if (g.attackWarnings) {
    for (const aw of g.attackWarnings) {
      ...
      m.children.forEach((child: AnyMesh) => {
  ...
  if (_cleanupFrameCounter >= CLEANUP_INTERVAL) {
    ...
    const disposeMesh = (mesh: AnyMesh): void => {
