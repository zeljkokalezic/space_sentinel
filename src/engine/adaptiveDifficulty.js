/**
 * adaptiveDifficulty.js — Dynamic difficulty adjustment based on player pressure.
 *
 ...
 * @returns {number} Pressure score clamped to [0, 1]
 */
export const calculatePressure = (g) => {
 ...
 */
export const updateAdaptiveDifficulty = (dt, g) => {
 ...
 */
export const getAdaptiveSpawnCooldown = (g, baseCooldown) => {
  const ad = g.adaptiveDifficulty;
 ...
 */
export const getAdaptiveAggression = (g) => {
  const ad = g.adaptiveDifficulty;
  if (!ad) return 1;
 ...
 * @param {Object} g — Game state
 */
export const recordMissionCompletion = (g) => {
  const ad = g.adaptiveDifficulty;
  if (!ad) return;
 ...
 * @param {Object} g — Game state
 */
export const resetRampageStreak = (g) => {
  const ad = g.adaptiveDifficulty;
  if (!ad) return;
