...
export const updatePhysics = (dt, g, cbs) => {
  ...
  if (g.escort.active && !g.mission.completed) {
    const esc = g.escort;
    ...
    if (esc.respawnTimer > 0) {
      ...
      // While respawning, skip movement but still check enemies
    } else if (esc.hp > 0) {
      ...
      if (esc.evasionTimer <= 0) {
        ...
        for (let p of g.projectiles) {
          ...
          if (pd < closestThreatDist) {
            // Check if projectile is moving toward drone
    ...
    for (let e of g.enemies) {
      ...
      const distToPlayer = Math.hypot(g.player.x - e.x, g.player.y - e.y);
      // If drone is closer, target it instead
