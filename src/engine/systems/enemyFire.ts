/**
 * enemyFire.ts — Shared enemy firing logic for both player and escort targets.
 *
...
/** Read escort velocity (not part of EscortState type) or player velocity. */
const targetVel = (g: GameState): { vx: number; vy: number } => {
  if (g.escort?.active) {
    const e = g.escort as unknown as { vx?: number; vy?: number };
    return { vx: e.vx || 0, vy: e.vy || 0 };
  }
  return { vx: g.player.vx, vy: g.player.vy };
};
 ...
 * @param angle — Angle toward target (radians)
 ...
 * @returns true if a weapon was fired (or warning started), false otherwise
 */
export const tryFireEnemyWeapon = (
  enemy: Enemy,
  angle: number,
  distToTarget: number,
  dt: number,
  ...
  const applyAggression = (baseCooldown: number): number => baseCooldown / adaptiveAggression;
  ...
  if (enemy.eliteVariant === 'sniper' && distToTarget < C.player.radius * C.eliteVariants.sniper.rangeMult) {
    ...
    const tv = targetVel(g);
    const predictedX = (getEnemyTarget(g).x) + tv.vx * travelTime;
    const predictedY = (getEnemyTarget(g).y) + tv.vy * travelTime;
  ...
  if (enemy.eliteVariant === 'swarmLeader' && distToTarget < C.player.radius * C.enemyWeapons.interceptor.rangeMult) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const impactX = targetX + Math.cos(burstAngle) * travelTime * tv.vx * 0.1;
      const impactY = targetY + Math.sin(burstAngle) * travelTime * tv.vy * 0.1;
