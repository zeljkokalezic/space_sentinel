/**
 * systems/enemies.ts — Enemy AI movement, firing, collision.
 *
...
import { createParticles, killEnemy, triggerScreenShake, triggerHitStop, triggerPlayerIFrames, applyDamageWithShield, spawnDamageNumber, isShieldBypassedByArmorPierce } from '../combat';
...
type FormationHandler = (dt: number, e: Enemy, g: GameState, tx: number, ty: number, dist: number) => void;
 ...
 */
function updateVanguard(dt: number, e: Enemy, g: GameState, tx: number, ty: number): void {
 ...
 */
function updateOrbit(dt: number, e: Enemy, g: GameState, tx: number, ty: number, dist: number): void {
 ...
 */
function updateSwarm(dt: number, e: Enemy, g: GameState, tx: number, ty: number): void {
  ...
  for (const other of g.enemies as Enemy[]) {
 ...
 */
function updateKamikaze(dt: number, e: Enemy, g: GameState, tx: number, ty: number): void {
 ...
 */
function updateBomber(dt: number, e: Enemy, g: GameState, tx: number, ty: number, dist: number): void {
  ...
  if (e.formationPhase === 'approach') {
    ...
    e.burstCooldown = (e.burstCooldown ?? 0) - dt;
    if (e.burstCooldown <= 0 && (e.burstCount ?? 0) < C.fireBurstCount) {
      // Fire spread shots via the enemy fire system
      const spread = ((e.burstCount ?? 0) - (C.fireBurstCount - 1) / 2) * 0.3;
      tryFireEnemyWeapon(e as Parameters<typeof tryFireEnemyWeapon>[0], angle + spread, dist, 0, 1, g);
 ...
 */
function updateScreen(dt: number, e: Enemy, g: GameState, tx: number, ty: number): void {
 ...
 */
function updateLegacy(dt: number, e: Enemy, g: GameState, tx: number, ty: number, distToTarget: number): void {
...
const FORMATION_HANDLERS: Record<string, FormationHandler> = {
...
export const updateEnemies = (
  ...
  for (const e of g.enemies as Enemy[]) {
    ...
    const angle = Math.atan2(ty - e.y, tx - e.x);
    if (e.type !== 'mini_interceptor') {
      tryFireEnemyWeapon(e as Parameters<typeof tryFireEnemyWeapon>[0], angle, distToTarget, dt, currentDiffMult, g, adaptiveAggression);
    }
    ...
    if (Math.hypot(e.x - g.player.x, e.y - g.player.y) < e.radius + g.player.radius) {
      ...
      if (g.playerIFrames && g.playerIFrames.isInvincible) {
        ...
        const { actualDmg: playerDmg, shieldAbsorbed: playerShieldDmg } = applyDamageWithShield(g, g.player, baseDmg * currentDiffMult, g.player.x, g.player.y);
        if (playerShieldDmg > 0) SoundManager.play('shield_hit');
        else SoundManager.play('player_hit');
        triggerScreenShake(g, 'playerHit');
        triggerHitStop(g, 'playerHit');
        triggerPlayerIFrames(g);
        applyDamageWithShield(g, e, C.weapons.missiles.baseDamage, e.x, e.y, { skipShield: isShieldBypassedByArmorPierce(e) });
        spawnDamageNumber(g, g.player.x, g.player.y - 10, playerDmg, { hitType: 'playerHit', shieldDamage: playerShieldDmg });
    ...
    // ── Enemy dies ──
    if (e.hp <= 0) {
      killEnemy(g, e as unknown as Record<string, unknown>, completeMission, e.lastHitBy);
