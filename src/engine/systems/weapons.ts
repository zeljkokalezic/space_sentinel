/**
 * systems/weapons.ts — Player weapon firing logic (autocannon, plasma, missiles, pointDefense).
 */
...
type SynergyFlags = Parameters<typeof fireProjectile>[8];

function rollUnstableReactorSelfDamage(g: GameState, chance: number): void {
 ...
 */
export const updateWeapons = (dt: number, g: GameState, completeMission?: () => void): void => {
  ...
  const aimAngle = g.player.aimAngle ?? 0;
  ...
  if (g.levels.autocannon > 0 && g.cooldowns.autocannon <= 0 && hasTarget) {
    ...
    for (let i = 0; i < shots; i++) {
      ...
      const pConfig: Record<string, unknown> = {};
      applyAutocannonSynergy(pConfig, activeSynergies);
      fireProjectile(g, bx, by, angle, C.weapons.autocannon.speed + (Math.random() * C.weapons.autocannon.speedVariance), dmg, 'autocannon', 0, pConfig as SynergyFlags);
    }
  ...
  if (g.levels.plasma > 0 && g.cooldowns.plasma <= 0 && hasTarget) {
    ...
    for (let i = 0; i < shots; i++) {
      ...
      fireProjectile(g, bx, by, angle, C.weapons.plasma.baseSpeed, (C.weapons.plasma.baseDamage + g.levels.plasma * C.weapons.plasma.damagePerLevel) * dmgMult * relicDmgMult * relicPlasmaMult, 'plasma', 1 + Math.floor(g.levels.plasma / 2), pConfig as SynergyFlags);
    }
  ...
  if (g.levels.pointDefense > 0 && g.cooldowns.pointDefense <= 0) {
    ...
    for (const m of g.projectiles as Projectile[]) {
