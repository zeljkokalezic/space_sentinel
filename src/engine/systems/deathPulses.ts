/**
 * systems/deathPulses.ts — Enemy death pulse shockwave system.
 *
...
import type { GameState, DeathPulseEntity } from '../state';

type Enemy = Record<string, unknown> & {
  active: boolean;
  x: number; y: number; radius?: number;
  hp: number; shield: number; maxShield: number;
  _hitByDeathPulses?: Set<DeathPulseEntity>;
};
 ...
 * @param g — Game state
 * @param completeMission — Optional callback to complete the current mission
 * @returns Always false (death pulses don't end the game)
 */
export const updateDeathPulses = (dt: number, g: GameState, completeMission?: () => void): boolean => {
    ...
    if (!pulse.hasDamagedPlayer && g.player) {
      ...
      if (distToPlayer <= pulse.radius + (g.player.radius || 38)) {
        ...
        if (g.playerIFrames && g.playerIFrames.isInvincible) {
          ...
          applyDamageWithShield(g, g.player, pulse.damage, g.player.x, g.player.y);
