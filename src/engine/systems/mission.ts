/**
 * systems/mission.ts — Mission completion detection, rewards, map progression, transition timer.
 */
...
import type { GameState, LevelState, StatsState } from '../state';

/** React state-sync callbacks supplied by the game loop. */
 ...
 * @param dt — Delta time
 * @param g — Game state
 * @param cbs — { setGameState, setMapStateVersion }
 * @returns true if transition is active (should skip other updates)
 */
export const updateTransition = (dt: number, g: GameState, cbs: MissionCallbacks): boolean => {
  if (g.transitionTimer === undefined) return false;

  g.transitionTimer = (g.transitionTimer as number) - dt;
  if ((g.transitionTimer as number) <= 0) {
    if (g.isVictory) {
 ...
 */
export const checkMissionProgress = (dt: number, g: GameState, completeMission: () => void): void => {
 ...
 * @param g — Game state
 * @param cbs — Optional callbacks { setMapStateVersion } for React state sync
 */
export const createCompleteMission = (g: GameState, cbs?: Partial<MissionCallbacks>): (() => void) => {
  const C = GAME_CONFIG;
  return () => {
    if (!g.mission || g.mission.completed) return;
    ...
    const enemiesAtStart = (g.missionStartStats?.enemiesDestroyed as number) ?? g.stats?.enemiesDestroyed ?? 0;
    ...
    if (g.map.currentNodeId) {
      const cur = g.map.nodes.find(n => n.id === g.map.currentNodeId);
      if (cur) cur.status = 'cleared';

      const nextEdges = g.map.edges.filter(e => e.from === g.map.currentNodeId);
      nextEdges.forEach(e => {
        const n = g.map.nodes.find(node => node.id === e.to);
 ...
 */
function countUpgradesMaxed(levels: LevelState): number {
  let count = 0;
  for (const key of Object.keys(levels)) {
    const maxLevel = (UPGRADE_DATA as Record<string, { maxLevel?: number }>)[key]?.maxLevel;
    if (maxLevel !== undefined && (levels as unknown as Record<string, number>)[key] >= maxLevel) count++;
  }
  return count;
}
 ...
 */
export const triggerGameOver = (): void => {
