/**
 * physics.ts — Main game-loop physics & simulation step.
...
/** React state-sync callbacks supplied by the game loop (useGameLoop). */
...
/** GameState fields touched here that live behind the index signature. */
type PhysicsState = GameState & {
  _lastNotifVersion?: number;
  spawnCooldown: number;
  worldMouse: { x: number; y: number };
  touchBase?: unknown;
  touchCurrent?: unknown;
};

/**
 * @param dt  - Delta time in seconds
 * @param g   - Live game state (game.current)
 * @param cbs - { setGameState, setMapStateVersion, setNotificationVersion }
 */
export const updatePhysics = (dt: number, gState: GameState, cbs: PhysicsCallbacks): void => {
  const g = gState as PhysicsState;
  ...
  updateAdaptiveDifficulty(dt, g as unknown as Parameters<typeof updateAdaptiveDifficulty>[1]);
  ...
  if (g.levels.autoAim > 0) {
    ...
    if (ne) { adx = (ne.x as number) - g.player.x; ady = (ne.y as number) - g.player.y; }
  ...
  // ─── Weapon cooldowns ─────────────────────────────────────────────────────────
  for (const k in g.cooldowns) (g.cooldowns as unknown as Record<string, number>)[k] -= dt;
