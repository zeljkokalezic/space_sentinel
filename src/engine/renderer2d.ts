...
/** A 2D canvas element with a cached rendering context (set on first draw). */
type CanvasWithCtx = HTMLCanvasElement & { _ctx?: CanvasRenderingContext2D | null };

/** Ref-like holder for the current game-state string. */
interface StatusRef { current: string }

/** projectToScreen signature from renderer3d. */
type ProjectFn = (camera: Any, wx: number, wy: number, wz?: number) => { x: number; y: number; visible: boolean };
...
const toRTransform = (wx: number, wy: number, pX: number, pY: number, rX: number, rY: number, rRange: number, rR: number, rRot: number): { px: number; py: number } => {
 ...
 */
export const draw2DFrame = (camera: Any, gState: GameState, canvasElArg: HTMLCanvasElement, statusRef: StatusRef, projectFn: ProjectFn): void => {
  const g = gState as Any;
  const canvasEl = canvasElArg as CanvasWithCtx;
  ...
  const c = canvasEl._ctx as CanvasRenderingContext2D;
  c.clearRect(0, 0, w, h);
  ...
  if (g.sector?.activeBuff && buffNames[g.sector.activeBuff as keyof typeof buffNames]) {
    c.fillStyle = 'rgba(168,85,247,0.9)';
    c.font = 'bold 10px monospace';
    c.textAlign = 'left';
    c.fillText(buffNames[g.sector.activeBuff as keyof typeof buffNames], 20, g.sector.veteranMode ? 76 : 64);
  }
  ...
  if (g.combo && g.combo.count > 0) {
    ...
    const comboColor = comboColors[g.combo.multiplier as keyof typeof comboColors] || '#ffffff'
  ...
  if (g.activeBuffs) {
    for (const [type, buff] of Object.entries(g.activeBuffs)) {
      if ((buff as any).timer > 0) {
        const cfg = C.powerups?.types?.[type as keyof typeof C.powerups.types];
        if (cfg) {
          c.fillStyle = cfg.color;
          c.font = 'bold 11px monospace';
          c.textAlign = 'left';
          c.fillText(`${cfg.icon} ${type.toUpperCase()} ${Math.ceil((buff as any).timer)}s`, 20, warnY);
          warnY += 16;
        }
      }
    }
 ...
 // ── Weather effect indicators ──────────────────────────────────────────────
  if (g.weather && g.weather.active.length > 0) {
    ...
    if (weather.active.includes('debrisField')) {
      const activeClusters = weather.debris.filter((d: Any) => d.active).length;
      c.fillStyle = '#9ca3af';
      c.font = 'bold 11px monospace';
      c.textAlign = 'left';
    ...
    if (weather.active.includes('gravityAnomaly')) {
      const activeZones = weather.gravityZones.filter((z: Any) => z.active).length;
      c.fillStyle = '#7c3aed';
      c.font = 'bold 11px monospace';
      c.textAlign = 'left';
  ...
  [0.33,0.66,1.0].forEach((f: number)=>{
  ...
  const toR = (wx: number, wy: number) => toRTransform(wx, wy, g.player.x, g.player.y, rX, rY, rRange, rR, rRot);
  ...
  for (let e of g.enemies) {
    ...
    const eColor = e.eliteVariant
      ? (radarCfg.enemyColors[e.eliteVariant as keyof typeof radarCfg.enemyColors] || radarCfg.enemyColors[e.type as keyof typeof radarCfg.enemyColors] || '#ef4444')
      : (radarCfg.enemyColors[e.type as keyof typeof radarCfg.enemyColors] || '#ef4444');
