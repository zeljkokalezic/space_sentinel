...
/** A generated sound: total duration, the audio nodes created, and an optional stop fn for looping sounds. */
...
/** Signature of every per-sound generator. */
type SoundGenerator = (ctx: AudioContext, gainNode: GainNode, now: number) => SoundResult;

type SoundtrackIntensity = 'calm' | 'tense' | 'triumphant';
 ...
 */
function applyEnvelope(gainNode: GainNode, attack: number, hold: number, decay: number, now: number): number {
  const t = now;
...
function playShoot(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
...
function playShootPlasma(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
  const osc = ctx.createOscillator();
  osc.type = 'sawtooth';
...
function playShootMissile(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
  const osc = ctx.createOscillator();
  osc.type = 'sine';
...
/** hit — Crack/pop */
function playHit(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
  const osc = ctx.createOscillator();
  osc.type = 'triangle';
...
function playExplosion(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
...
/** pickup — High ding */
function playPickup(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
  const osc = ctx.createOscillator();
  osc.type = 'sine';
...
function playShieldHit(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
  const osc = ctx.createOscillator();
  osc.type = 'sawtooth';
...
/** player_hit — Low thud */
function playPlayerHit(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
  const osc = ctx.createOscillator();
  osc.type = 'sine';
...
function playMissionComplete(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
...
function playGameOver(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
  const osc = ctx.createOscillator();
  osc.type = 'sine';
...
function playComboMilestone(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
...
function playPowerupPickup(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
...
function playEngine(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
...
function playBgDrone(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
  const osc = ctx.createOscillator();
  osc.type = 'sine';
...
function _playSoundtrackCalm(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
...
function _playSoundtrackTense(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
  // Low pulsing bass
...
function _playSoundtrackTriumphant(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
...
/** ui_click — Short blip */
function playUiClick(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
  const osc = ctx.createOscillator();
  osc.type = 'sine';
...
function playBossPhaseChange(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
  const osc = ctx.createOscillator();
  osc.type = 'sawtooth';
...
function playBossRage(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
...
function playEnemyShoot(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
  const osc = ctx.createOscillator();
  osc.type = 'square';
...
function playWaveAnnounce(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
  const osc = ctx.createOscillator();
  osc.type = 'sine';
...
function playCountdownBeep(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
  const osc = ctx.createOscillator();
  osc.type = 'sine';
  osc.frequency.setValueAtTime(880, now);
  osc.connect(gainNode);
...
function playWaveStart(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
  const osc = ctx.createOscillator();
  osc.type = 'square';
...
function playBossSpawn(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
  // Low noise rumble
...
function playBossIntro(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
...
function playHeartbeat(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
...
function playShieldBreak(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
...
function playShieldRestore(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
...
function playEnemySpawn(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
  const osc = ctx.createOscillator();
  osc.type = 'triangle';
  osc.frequency.setValueAtTime(600, now);
...
function playScrapCollect(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
  // High metallic ping
  const osc1 = ctx.createOscillator();
  osc1.type = 'sine';
...
function playRelicAcquired(ctx: AudioContext, gainNode: GainNode, now: number): SoundResult {
  // Main chime (sine, C5)
  const osc1 = ctx.createOscillator();
  osc1.type = 'sine';
...
const SOUND_GENERATORS: Record<string, SoundGenerator> = {
  shoot: playShoot,
...
class SoundManagerClass {
  ...
  private _continuous: Record<string, ContinuousEntry> = {};
  private _pendingTimeouts: ReturnType<typeof setTimeout>[] = [];
  private _soundtrackIntensity: SoundtrackIntensity = 'calm';
  private _soundtrackActive = false;
  private _pausedVolume?: number;
   ...
   */
  setSoundtrackIntensity(intensity: SoundtrackIntensity): void {
   ...
   */
  startSoundtrack(initialIntensity: SoundtrackIntensity = 'calm'): void {
    if (this._soundtrackActive) return;
    this._soundtrackActive = true;
    this._soundtrackIntensity = initialIntensity;
    this._startSoundtrackLayer(initialIntensity, this.ctx?.currentTime ?? 0, 0);
  }
  ...
  /**
   * @returns Whether the soundtrack is active.
   */
  ...
  /**
   * @returns Current soundtrack intensity.
   */
  getSoundtrackIntensity(): SoundtrackIntensity {
   ...
   */
  private _startSoundtrackLayer(intensity: SoundtrackIntensity, now: number, fadeDuration: number): void {
   ...
   */
  private _stopSoundtrackLayer(intensity: SoundtrackIntensity, now: number, fadeDuration: number): void {
    const name = `soundtrack_${intensity}`;
  ...
  private _stopContinuous(name: string): void {
    const entry = this._continuous[name];
    if (!entry) return;
    try {
...
export { SoundManager };
export type { SoundtrackIntensity };
