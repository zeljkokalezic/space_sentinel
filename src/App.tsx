...
import type { GameStateName, GameRef } from './components/types';
import type { EmergencyBeaconState } from './engine/state';
import type { DevMissionConfig } from './components/DevMissionPicker';
...
export default function App() {
  // ─── React state ────────────────────────────────────────────────────────────
  const [gameState,       setGameState]       = useState<GameStateName>('start');
  const [uiScrap,         setUiScrap]         = useState(0);
  const [uiLevels,        setUiLevels]        = useState<Record<string, number>>({});
  const [uiShipSkin,      setUiShipSkin]      = useState(0);
  const [uiUnlockedSkins, setUiUnlockedSkins] = useState<boolean[]>(() => SHIP_SKINS.map(s => s.cost === 0));
  const [mapStateVersion, setMapStateVersion] = useState(0);
  const [notificationVersion, setNotificationVersion] = useState(0);
  const [devMode,         setDevMode]         = useState(false);
  const [paused,          setPaused]          = useState(false);
  const [uiEmergencyBeacon, setUiEmergencyBeacon] = useState<EmergencyBeaconState>({ purchased: false, activated: false, nodeId: null });

  // ─── Mutable refs (shared across hooks) ─────────────────────────────────────
  const containerRef = useRef<HTMLDivElement | null>(null);
  const canvasRef    = useRef<HTMLCanvasElement | null>(null);
  const game: GameRef = useRef(null);
  ...
  const selectStartingRelic = (relicId: string) => {
    if (game.current) tryAddRelic(game.current, relicId);
    setGameState('map');
  };
  ...
  const continueGame = () => {
    resetGame();
    if (!game.current || !loadGame(game.current, 'auto')) return;
  ...
  const launchDevMission = ({ type, level, hazard, variantIndex }: DevMissionConfig) => {
    resetGame();
    const g = game.current;
    if (!g) return;
    g.level = level;
  ...
  const buyUpgrade = (key: string, cost: number) => {
    const g = game.current;
    if (!g) return;
    const upgrade = (UPGRADE_DATA as Record<string, { maxLevel: number }>)[key];
    const levels = g.levels as unknown as Record<string, number>;
    if (!upgrade || levels[key] >= upgrade.maxLevel) return;
  ...
  const effectiveSetState = (state: GameStateName) => {
    if (state === 'gameover' && game.current?.emergencyBeacon?.activated) {
      ...
      setUiLevels({ ...(g.levels as unknown as Record<string, number>) });
      setUiScrap(g.scrap);
      setGameState('map');
      return;
    }
  ...
  const { onPointerDown, onPointerMove, onPointerUp } = useInput({
    game, threeRef, gameState, statusRef, setGameState, setDevMode, startGame, setPaused, setUiEmergencyBeacon,
  });
