...
import type { GameSettings, DifficultySetting, ParticleQuality, ColorblindMode } from '../engine/settings';
...
// Boolean-valued setting keys (used by handleToggle)
type BooleanSettingKey = 'showFPS' | 'screenShake' | 'showRadarLegend' | 'reducedMotion' | 'highContrast';
 ...
 */
export default function SettingsOverlay({ gameRef, onClose }: SettingsOverlayProps) {
  ...
  const handleDifficultyChange = (val: DifficultySetting) => {
  ...
  const handleColorblindMode = (val: ColorblindMode) => {
    const next = { ...settings, colorblindMode: val };
    setSettings(next);
    saveSettings(next);
  };

  const handleParticlesQuality = (val: ParticleQuality) => {
      ...
      <div style={{
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <div style={{ marginBottom: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              {(['easy', 'normal', 'hard'] as DifficultySetting[]).map((d) => (
                <button
                  key={d}
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <div style={{ marginBottom: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              {(['low', 'medium', 'high'] as ParticleQuality[]).map((q) => (
                <button
                  key={q}
