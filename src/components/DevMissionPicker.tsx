...
interface ColorClasses { border: string; bg: string; text: string; glow: string }

interface BossVariant { id: string; name: string; color: number; [key: string]: unknown }

export interface DevMissionConfig {
  type: string;
  level: number;
  hazard: string;
  label: string;
...
interface DevMissionPickerProps {
  onLaunch: (config: DevMissionConfig) => void;
  onExit: () => void;
}

const HAZARD_OPTIONS: { id: string; label: string; icon: ComponentType<{ className?: string }> | null }[] = [
...
function MissionCard({ mission, selected, onClick }: { mission: MissionType; selected: boolean; onClick: () => void }) {
...
function LevelSlider({ value, onChange, min = 1, max = 20 }: { value: number; onChange: (v: number) => void; min?: number; max?: number }) {
  return (
...
function VariantSelector({ roster, value, onChange, label }: { roster: BossVariant[]; value: number; onChange: (i: number) => void; label: string }) {
  return (
    <div className="flex flex-col items-center gap-2 w-full max-w-xs">
...
export default function DevMissionPicker({ onLaunch, onExit }: DevMissionPickerProps) {
  ...
  const roster = (selectedType === 'kill_boss' ? BOSS_ROSTER : MINIBOSS_ROSTER) as unknown as BossVariant[];
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-2 rounded-lg border transition-all text-xs font-bold
                  ${(colors as Record<string, string>)[h.id] || colors.none}
