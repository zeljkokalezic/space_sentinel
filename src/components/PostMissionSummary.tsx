/**
 * PostMissionSummary.tsx — Post-mission statistics overlay.
 *
 ...
 * @returns Rating letter (S, A, B, C, D)
 */
function calculateRating(stats: MissionSummary): string {
...
interface PostMissionSummaryProps {
  game: GameRef;
  visible: boolean;
}

export default function PostMissionSummary({ game, visible }: PostMissionSummaryProps) {
  if (!visible || !game?.current) return null;
  const g = game.current;
  const stats = g.lastMissionSummary as unknown as MissionSummary | null;
