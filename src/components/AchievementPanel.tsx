...
import type { AchievementDef, AchievementProgress } from '../engine/achievements';
import { Trophy, Award, Lock, ChevronLeft } from 'lucide-react';
import type { GameRef } from './types';

const FILTERS: { key: string; label: string; icon: typeof Trophy }[] = [
 ...
 */
function buildStats(g: Record<string, unknown> | null | undefined): Record<string, number> {
  const s = (g?.stats as Record<string, unknown>) || {};
  return {
    enemiesDestroyed: (s.enemiesDestroyed as number) || 0,
    totalScrap: (s.totalScrap as number) || 0,
    surviveMissions: (s.surviveMissions as number) || 0,
    escortMissions: (s.escortMissions as number) || 0,
    defendMissions: (s.defendMissions as number) || 0,
    sabotageMissions: (s.sabotageMissions as number) || 0,
    bossesDefeated: (s.bossesDefeated as number) || 0,
    minibossesDefeated: (s.minibossesDefeated as number) || 0,
    upgradesMaxed: (s.upgradesMaxed as number) || 0,
...
function AchievementCard({ achievement, unlocked, progress }: AchievementCardProps) {
...
export default function AchievementPanel({ game, onClose }: AchievementPanelProps) {
  const [filter, setFilter] = useState('all');

  // Re-read data on each render so progress is fresh
  const unlocked: Set<string> = game?.current?.achievements?.unlocked || new Set();
  const stats = buildStats(game?.current);
  const progressList: AchievementProgress[] = getAchievementProgress(unlocked, stats);
