...
import type { SectorRankResult } from '../engine/sectorRank';
...
export default function VictoryScreen({ gameRef, startGame, nextSector }: VictoryScreenProps) {
  const g = gameRef?.current;

  const [rankData, setRankData] = useState<SectorRankResult | null>(null);
  const [buffChoices, setBuffChoices] = useState<BuffChoice[]>([]);
  ...
  const handleBuffSelect = (buffId: string) => {
    if (!g) return;
  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center bg-cyan-950/90 text-white z-50 backdrop-blur-md overflow-y-auto">
      <div className="py-8 px-4 w-full max-w-lg">
        ...
        {g?.sector?.consecutiveARank && g.sector.consecutiveARank > 0 && (
