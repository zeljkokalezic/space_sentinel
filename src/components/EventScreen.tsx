...
import type { SpaceEvent, EventChoice, EventGameRef } from '../constants/events';
import { tryAddRelic, getRandomRelic } from '../engine/relicSystem';
import type { RelicDef } from '../engine/relicSystem';
...
export default function EventScreen({ gameRef, setGameState, setUiScrap, setUiLevels }: EventScreenProps) {
  const [eventData, setEventData] = useState<SpaceEvent | null>(null);
  const [relicAcquired, setRelicAcquired] = useState<RelicDef | null>(null);
  ...
  const eventRef = gameRef as unknown as EventGameRef;

  const handleChoice = (choice: EventChoice) => {
    // Execute the consequence
    if (choice.resolve) {
      choice.resolve(eventRef);
    }

    // Handle relic reward
    if (choice.relicReward) {
      const relic = getRandomRelic(choice.relicReward.rarity);
      if (relic && gameRef.current && tryAddRelic(gameRef.current, relic.id)) {
    ...
    <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/95 text-white z-50 backdrop-blur-lg">
      ...
      <div className="flex flex-col gap-4 w-full max-w-2xl">
        {eventData.choices.map((choice, idx) => {
          const isAvailable = choice.condition ? choice.condition(eventRef) : true;
