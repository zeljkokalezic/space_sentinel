...
import type { GameStateName, GameRef } from './types';

type MapNode = GameState['map']['nodes'][number];

interface MapOverlayProps {
  game: GameState | null;
  setGameState: (state: GameStateName) => void;
  setUiScrap: (scrap: number) => void;
  setUiLevels: (levels: Record<string, number>) => void;
  setUiShipSkin?: (idx: number) => void;
  setUiUnlockedSkins?: (skins: boolean[]) => void;
  setMapStateVersion: (updater: (v: number) => number) => void;
  mapStateVersion?: number;
  setUiEmergencyBeacon?: (beacon: { purchased: boolean; activated: boolean; nodeId: string | null }) => void;
}
 ...
 */
export default function MapOverlay({ game, setGameState, setUiScrap, setUiLevels, setUiShipSkin, setUiUnlockedSkins, setMapStateVersion, setUiEmergencyBeacon }: MapOverlayProps) {
  ...
  const getColor = (type: string, status: string): string => {
  ...
  const handleNodeClick = (n: MapNode) => {
    ...
    if (n.type === 'shop') {
      ...
      enterNodeMission(game, game.level, n.type, n as unknown as Record<string, unknown>);
      ...
      {game.relics && game.relics.length > 0 && (
        <div className="absolute bottom-8 right-4 bg-[#0a0a14]/90 border border-gray-700 rounded-xl p-5 font-sans text-sm shadow-2xl z-50 backdrop-blur-md">
          ...
          <div className="flex flex-wrap gap-2 max-w-48">
            {game.relics.map((rId: string, i: number) => {
      ...
      {nodes.map(n => {
        ...
        return (
          ...
          >
            ...
            {/* Hazard badges */}
            {isAvailable && n.hazardTypes && n.hazardTypes.map((ht: string, i: number) => (
