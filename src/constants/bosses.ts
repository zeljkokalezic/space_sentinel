/**
 * bosses.ts — Boss and mini-boss roster data.
 *
...
import type { AttackPatternKey } from './attackPatterns';

export type BossGeometry = 'box' | 'dodecahedron' | 'octahedron' | 'tetrahedron' | 'icosahedron';
export type BossPowerupDrop = 'shieldBoost' | 'damageSurge';

export interface BossAttackPatternSet {
  phase1: AttackPatternKey;
  phase2: AttackPatternKey;
  phase3: AttackPatternKey;
}

export interface VoidZonesSignature {
  type: 'void_zones';
...
export interface ShieldRegenSignature {
  type: 'shield_regen';
  regenPercentPerSecond: number;
  noDamageThreshold: number;
  visualBuildupTime: number;
}

export interface PhaseShiftSignature {
  type: 'phase_shift';
...
export type BossSignatureMechanic =
  | VoidZonesSignature
  | ShieldRegenSignature
  | PhaseShiftSignature;

interface BossVariantBase {
  id: string;
  name: string;
  title: string;
  introText: string;
  ...
  attackPatterns: BossAttackPatternSet;
  deathColors: number[];
  guaranteedDrops: BossPowerupDrop[] | null;
  scrapReward: number;
}

export interface BossVariant extends BossVariantBase {
  baseHp: number;
  hpPerLevel: number;
  signatureMechanic?: BossSignatureMechanic;
}

export interface MinibossVariant extends BossVariantBase {
  hpPercent: number;
}
 ...
 */
export const BOSS_ROSTER: BossVariant[] = [
  {
    id: 'void_reaper',
    name: 'Void Reaper',
 ...
 */
export const MINIBOSS_ROSTER: MinibossVariant[] = [
  {
    id: 'scout_alpha',
    name: 'Scout Alpha',
