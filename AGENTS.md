...
- `useGameLoop.jsx`: Three.js scene init, animation frame loop, and resize handling. Returns `{ threeRef, statusRef, devModeRef, physicsCbs }` where `physicsCbs` is the callback object for `updatePhysics`. Uses a ref (`physicsCbsRef`) to ensure the game loop closure always sees the latest callbacks without stale closure issues. Manages `requestAnimationFrame` lifecycle and calls `updatePhysics()` + `drawFrame()` each tick.
- `useInput.jsx`: Keyboard and pointer event handlers. Returns `{ onPointerDown, onPointerMove, onPointerUp }` for attaching to the canvas container. Handles WASD/arrows, Q/E strafe, space (shop->map), ESC pause, B emergency beacon activation (syncs `uiEmergencyBeacon` state), backtick (dev mode toggle), and mouse/touch input for ship aiming and joystick.
...
- `bossSetup.js`: Boss fight initialization — `setupBoss(g, level)`, `resetBoss(g)`, `setupBossCore(g, level, roster, config)` (shared by boss and miniboss). Selects variant from `BOSS_ROSTER`, spreads properties, triggers intro effects + spawn sound.
